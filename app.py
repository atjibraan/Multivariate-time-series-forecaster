...
class FixedMultiHeadAttention(tf.keras.layers.MultiHeadAttention):
    def __init__(self, **kwargs):
        # Remove problematic parameters during loading
...
try:
    model = load_fixed_model("transformer_forecaster.h5")
...
if uploaded_file is not None:
    try:
        ...
        for i, col in enumerate(target_cols):
            ax = axes[i]
            ax.plot(results['Timestamp'], results[col], marker='o', linestyle='-', color='blue')
        ...
        for i, col in enumerate(target_cols):
            ax = axes2[i]
            # Last 72 hours of historical data
            hist_trimmed = history.iloc[-72:]
            ax.plot(hist_trimmed['timestamp'], hist_trimmed[col], label='Historical', color='green')
            ax.plot(forecast['Timestamp'], forecast[col], label='Forecast', marker='o', color='blue')
